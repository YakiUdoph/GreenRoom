...
export function ManusHeader({ activeTab, onTabChange }) {
  ...
  return <>
    <header className="site-header">
      ...
      <div className="header-actions"><button type="button" className="header-docs" onClick={() => go('docs')}>DOCS</button><button type="button" className="menu-control" onClick={() => setOpen(v => !v)} aria-expanded={open} aria-label="Toggle navigation">{open ? '×' : '☰'}</button></div>
    </header>
