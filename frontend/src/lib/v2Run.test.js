...
test('reload polling stops only at truthful terminal V2 states', () => { assert.equal(shouldPollV2Run({ run_id: 'run_v2_123456abcdef', status: 'WAITING_FOR_MINDS' }), true); assert.equal(shouldPollV2Run({ run_id: 'run_v2_123456abcdef', status: 'COMPLETED' }), false); });
