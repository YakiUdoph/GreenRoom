...
const lifecycle = [
  ...
  ['04 — INTELLIGENCE', 'A completed live return keeps its first-party source, deterministic decision fields, and run identity attached.'],
...
function SectionBody({ id }) {
  if (id === 'overview') return <><DocHead label="PRODUCT THESIS" title="Persistent decision intelligence for creators."/><p>GreenRoom helps solo creators carry goals, preferences, constraints, and prior decisions from one session to the next. AI video is the first supported live vertical, not the scope of the product. Future writing, design, audio, publishing, and growth domains require their own verified providers.</p><h3>The product promise</h3><div className="docs-feature-grid">{['Remembers how you work','Keeps an objective active','Works in the background','Returns an actionable briefing','Learns from feedback'].map(item => <span key={item}>{item}</span>)}</div></>;
  ...
  if (id === 'background') return <><DocHead label="HONEST LIFECYCLE" title="Leave the screen. Keep the thread."/><p>Production runs through the existing signed background worker and durable persistence. The creator-facing contract is intentionally simpler than the infrastructure:</p><div className="docs-state-row"><span>QUEUED</span><span>WORKING</span><span>RESULT READY</span><span>NO RELEVANT UPDATE</span><span>RUN FAILED</span></div><p>Closing the lifecycle view does not cancel durable work. Waiting and prior briefings are never presented as a new result.</p></>;
