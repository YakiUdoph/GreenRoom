...
export function IntelligencePage({ memoryState }) {
  ...
  const currentV2RunId = memoryState?.current_v2_run?.run_id;
  ...
  useEffect(() => { let disposed = false; Promise.all([api.getRecentBriefingRuns(), api.getV2History().catch(() => ({ runs: [] }))]).then(async ([recent, v2History]) => {
    ...
    const v2 = await Promise.all((v2History.runs || []).filter(record => record.run_id !== currentV2RunId).map(async record => ({ kind: 'v2', record, status: record, briefing: await api.getV2Decision(record.run_id), snapshot: null })));
    if (!disposed) { setRuns([...v2, ...checked.filter(item => item.status === 'fulfilled' && item.value).map(item => item.value)].sort((a, b) => Date.parse(b.status.completed_at || 0) - Date.parse(a.status.completed_at || 0))); setState('ready'); }
  }).catch(() => !disposed && setState('error')); return () => { disposed = true; }; }, [currentRunId, currentV2RunId]);
  const choose = item => { setDetailState('loading'); setSelected(null); if (item.kind === 'v2') { api.getV2Decision(item.record.run_id).then(briefing => { if (briefing.run_id !== item.record.run_id) throw new Error('Run mismatch'); setSelected({ ...item, briefing }); setDetailState('ready'); }).catch(() => setDetailState('error')); return; } Promise.all([api.getBriefingStatus(item.record.run_id), api.getRunBriefing(item.record.run_id)]).then(([status, response]) => { const verified = verifyHistoricalBriefing(item.record, status, response); if (isSimulatedBriefing(verified.briefing)) throw new Error('Simulated'); setSelected({ ...item, briefing: verified.briefing }); setDetailState('ready'); }).catch(() => setDetailState('error')); };
