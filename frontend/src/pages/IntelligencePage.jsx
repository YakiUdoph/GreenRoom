...
function BriefingFields({ briefing }) {
  ...
  return <>
    ...
    <details className="briefing-proof"><summary>Verify proof</summary><dl><div><dt>EVIDENCE</dt><dd>{briefing.evidence_mode || provenance.evidence_mode || 'Not supplied'}</dd></div><div><dt>PROVIDER / CATEGORY</dt><dd>{[sources[0]?.source, sources[0]?.category].filter(Boolean).join(' / ') || 'See source below'}</dd></div><div><dt>MEMORY USED</dt><dd>{items[0]?.memory_context_used || 'No relevant Memory was reported.'}</dd></div><div><dt>PERSISTENT MIND</dt><dd>{briefing.minds_verified && provenance.minds_verified ? 'Udophia — verified for this run' : 'Not verified for this result'}</dd></div><div><dt>RUN</dt><dd>{briefing.run_id || 'Not supplied'}</dd></div><div><dt>PERSISTENCE</dt><dd>{briefing.persistence_mode || provenance.persistence_mode || 'Not supplied'}</dd></div></dl></details>
